...
export function ColorCard({ color }) {
  ...
  return (
    ...
    >
      <CLink href={`/product/${color.product_slug}`}>
        <div className="relative bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 transform hover:-translate-y-2">
          <div className="relative w-full aspect-[205/200] overflow-hidden">
            <Image
              src={`https://app.intercocina.com/storage/${color.image}`}
              alt={color.name}
              width={200}
              height={300}
              className={`h-full w-full object-cover object-top transition-transform duration-500 ${isHovered ? 'scale-110' : 'scale-100'
                }`}
            />
